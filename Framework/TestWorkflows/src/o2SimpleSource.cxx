...
WorkflowSpec defineDataProcessing(ConfigContext const& ctx)
{
  ...
  std::vector<TimerInterval> timers;
  ...
  if (timer.empty() == false) {
    ...
    while (true) {
      ...
      auto duration = std::stoull(timer.substr(0, colon));
      auto interval = std::stoull(timer.substr(colon + 1, comma - colon - 1));
      timers.push_back(TimerInterval{duration, interval});
      if (comma == std::string::npos) {
        break;
      }
      timer = timer.substr(comma + 1);
    }
    inputs.emplace_back("timer", "TST", "TIMER", 0, Lifetime::Timer, timerIntervals(timers));
  }
