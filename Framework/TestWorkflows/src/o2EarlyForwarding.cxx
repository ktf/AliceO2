...
WorkflowSpec defineDataProcessing(ConfigContext const& specs)
{
  ...
  // Depends on only a and b which are not slow, however it should
  // still be after D, because of the expendable label.
  // This is to mimick the case in which early forwarding happens
  // immediately, rather than when the processing is ready to go.
  DataProcessorSpec e{.name = "E",
                      .inputs = {
                        InputSpec{"a", "TST", "A1"},
                        InputSpec{"b", "TST", "B1"},
                      },
                      .algorithm = AlgorithmSpec{adaptStateless([](InputRecord& inputs) {
                        auto ref = inputs.get("b");
                        auto header = o2::header::get<const DataProcessingHeader*>(ref.header);
                        LOG(info) << "Start time: " << header->startTime;
                      })},
