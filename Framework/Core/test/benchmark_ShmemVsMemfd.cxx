...
{
#ifdef __linux__
  return alignUp(size, HUGE_PAGE_SIZE);
#else
  return size;
#endif
}
...
{
#ifdef __linux__
  int fd = memfd_create("benchmark_region", MFD_CLOEXEC | MFD_HUGETLB);
  if (fd < 0) {
    // Fall back to regular pages if huge pages unavailable
    fd = memfd_create("benchmark_region", MFD_CLOEXEC);
  }
  if (fd < 0) {
    perror("memfd_create");
    return -1;
...
static ApproachBResult benchmarkMemfdUDS(const std::vector<size_t>& sizes)
{
  ...
  regionSize = alignToHugePage(regionSize);
