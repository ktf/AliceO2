...
// ---------------------------------------------------------------------------
// Approach C: slab-based memfd with oldest-possible-TF tracking
// ---------------------------------------------------------------------------
// Pre-create a few large slabs (memfds). Bump-allocate TFs into them.
// The receiver tracks the oldest possible TF and incrementally calls
// madvise(MADV_DONTNEED) on consumed regions. The sender determines
// slab availability from the oldest-possible-TF watermark.
...
// Oldest-possible-TF update: receiver tells sender which TFs are consumed
struct OldestTFUpdate {
  int32_t oldestPossibleTF; // all TFs with index < this are fully consumed
};

static bool sendOldestTF(int sockFd, const OldestTFUpdate& update)
{
  return send(sockFd, &update, sizeof(update), 0) == sizeof(update);
}

static bool recvOldestTF(int sockFd, OldestTFUpdate& update)
{
  return recv(sockFd, &update, sizeof(update), MSG_WAITALL) == sizeof(update);
}

// Per-slab tracking on the sender side
struct SenderSlabState {
  int lastTFIndex = -1; // last TF index placed in this slab
};
...
static ApproachCResult benchmarkSlabMemfd(const std::vector<size_t>& sizes)
{
  ...
  if (pid == 0) {
    ...
    // Per-slab high-water mark: how far we've madvised
    size_t slabAdvisedUpTo[N_SLABS] = {};
    ...
    for (int iter = 0; iter < N_ITERATIONS; ++iter) {
      ...
      // Advance oldest possible TF — this TF is consumed.
      // madvise consumed pages in this slab up to the end of this TF.
      double madvMs = 0.0;
      auto tm0 = Clock::now();
      {
        int si = manifest.slabIndex;
        size_t tfEnd = manifest.baseOffset + manifest.totalSize;

        // Detect slab reuse: if baseOffset is before our high-water mark,
        // the sender has recycled this slab — reset tracking.
        if (manifest.baseOffset < slabAdvisedUpTo[si]) {
          slabAdvisedUpTo[si] = 0;
        }

        // Page-align: only madvise complete pages
        size_t pageAlignedStart = (slabAdvisedUpTo[si] + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1);
        size_t pageAlignedEnd = tfEnd & ~(PAGE_SIZE - 1);
        if (pageAlignedEnd > pageAlignedStart) {
#ifdef MADV_DONTNEED
          madvise(static_cast<uint8_t*>(slabMaps[si]) + pageAlignedStart,
                  pageAlignedEnd - pageAlignedStart, MADV_DONTNEED);
#endif
        }
        slabAdvisedUpTo[si] = tfEnd;
      }
      auto tm1 = Clock::now();
      madvMs = msElapsed(tm0, tm1);

      // Send oldest-possible-TF update to sender
      OldestTFUpdate update{static_cast<int32_t>(iter + 1)};
      if (!sendOldestTF(sock, update)) {
        perror("sendOldestTF");
        _exit(1);
      }
  ...
  // Per-slab: track the last TF index stored in each slab
  SenderSlabState senderSlabs[N_SLABS] = {};
  int32_t knownOldestTF = 0; // latest oldest-possible-TF from receiver
  ...
  for (int iter = 0; iter < N_ITERATIONS; ++iter) {
    ...
    if (slabOffset + tfSize > SLAB_SIZE) {
      // Move to next slab
      int nextSlab = (currentSlab + 1) % N_SLABS;

      // A slab is available when oldestPossibleTF > lastTFIndex in that slab,
      // meaning all TFs that were in it have been consumed.
      while (senderSlabs[nextSlab].lastTFIndex >= 0 &&
             knownOldestTF <= senderSlabs[nextSlab].lastTFIndex) {
        OldestTFUpdate update{};
        if (!recvOldestTF(connSock, update)) {
          fprintf(stderr, "slab: recvOldestTF failed waiting for slab %d\n", nextSlab);
          exit(1);
        }
        knownOldestTF = update.oldestPossibleTF;
      }
    ...
    senderSlabs[currentSlab].lastTFIndex = iter;
    slabOffset += tfSize;

    totalFillMs += msElapsed(t0, t1);
    totalSendManifestMs += msElapsed(t1, t2);

    // Read one oldest-TF update per TF to stay in sync
    OldestTFUpdate update{};
    if (!recvOldestTF(connSock, update)) {
      fprintf(stderr, "slab: recvOldestTF failed at iter=%d\n", iter);
      exit(1);
    }
    knownOldestTF = update.oldestPossibleTF;
  }
