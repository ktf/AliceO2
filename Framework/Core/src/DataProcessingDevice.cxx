      ...
      if (copy) {
        for (auto& cachedForwardingChoice : cachedForwardingChoices) {
          ...
          O2_SIGNPOST_EVENT_EMIT(forwarding, sid, "forwardInputs", "Forwarding a copy of %{public}s to route %d.",
                                 fmt::format(fmt::runtime("{x}@timeslice:{} tfCounter:{}"), *dh, dph->startTime, dh->tfCounter).c_str(), cachedForwardingChoice.value);
        ...
        O2_SIGNPOST_EVENT_EMIT(forwarding, sid, "forwardInputs", "Forwarding %{public}s to route %d.",
                               fmt::format(fmt::runtime("{x}@timeslice:{} tfCounter:{}"), *dh, dph->startTime, dh->tfCounter).c_str(), cachedForwardingChoices.back().value);
