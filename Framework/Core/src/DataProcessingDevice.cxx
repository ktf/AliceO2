...
void DataProcessingDevice::handleData(ServiceRegistryRef ref, InputChannelInfo& info)
{
  ...
  auto getInputTypes = [&info, &context]() -> std::optional<std::vector<InputInfo>> {
    ...
    auto insertInputInfo = [&results, &nTotalPayloads](size_t position, size_t length, InputType type, char const* channelName) {
      results.emplace_back(position, length, type, channelName);
    ...
    for (size_t pi = 0; pi < parts.Size(); pi += 2) {
      ...
      if (sih) {
        ...
        insertInputInfo(pi, 2, InputType::SourceInfo, info.channel->GetName().c_str());
        *context.wasActive = true;
        continue;
      }
      auto dih = o2::header::get<DomainInfoHeader*>(headerData);
      if (dih) {
        insertInputInfo(pi, 2, InputType::DomainInfo, info.channel->GetName().c_str());
        *context.wasActive = true;
        continue;
      }
      auto dh = o2::header::get<DataHeader*>(headerData);
      if (!dh) {
        insertInputInfo(pi, 0, InputType::Invalid, info.channel->GetName().c_str());
      ...
      if (!dph) {
        insertInputInfo(pi, 2, InputType::Invalid, info.channel->GetName().c_str());
      ...
      if (dh->splitPayloadParts > 0 && dh->splitPayloadParts == dh->splitPayloadIndex) {
        ...
        insertInputInfo(pi, dh->splitPayloadParts + 1, InputType::Data, info.channel->GetName().c_str());
        ...
        insertInputInfo(pi, 2, InputType::Data, info.channel->GetName().c_str());
        for (; pi + 2 < finalSplitPayloadIndex; pi += 2) {
          insertInputInfo(pi + 2, 2, InputType::Data, info.channel->GetName().c_str());
        }
      }
    }
