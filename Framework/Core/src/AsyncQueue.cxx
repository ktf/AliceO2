...
auto AsyncQueueHelpers::run(AsyncQueue& queue, TimesliceId oldestPossible) -> void
{
  ...
  LOGP(info, "Attempting at running {} tasks", queue.tasks.size());
  std::vector<int> order;
  order.resize(queue.tasks.size());
  std::iota(order.begin(), order.end(), 0);

  // Sort by timeslice, then priority and finally debounce
  ...
  for (auto i : order) {
    if (queue.tasks[i].timeslice.value < oldestPossible.value) {
      LOGP(info, "AsyncQueue: Running task {}, timeslice {}, score {}, debounce {}", queue.tasks[i].id.value, queue.tasks[i].timeslice.value, queue.prototypes[queue.tasks[i].id.value].score, queue.tasks[i].debounce);
    } else {
      LOGP(info, "AsyncQueue: Skipping task {}, timeslice {}, score {}, debounce {}", queue.tasks[i].id.value, queue.tasks[i].timeslice.value, queue.prototypes[queue.tasks[i].id.value].score, queue.tasks[i].debounce);
    }
  }
