  ...
  auto generator = [this]() -> arrow::Future<std::shared_ptr<arrow::RecordBatch>> {
    ...
    for (auto& field : this->physical_schema_->fields()) {
      if (auto listType = std::dynamic_pointer_cast<arrow::FixedSizeListType>(field->type())) {
        ...
        auto vdata = std::make_shared<arrow::ArrayData>(field->type()->field(0)->type(), mRows * listType->list_size(), std::vector<std::shared_ptr<arrow::Buffer>>{GetPlaceholderForOp(size)});
        ...
        auto data = std::make_shared<arrow::ArrayData>(field->type(), mRows, std::vector<std::shared_ptr<arrow::Buffer>>{GetPlaceholderForOp(size)});
