...
void retryMetricCallback(uv_async_t* async)
{
  ...
  if (channel == device->GetChannels().end()) {
    O2_SIGNPOST_EVENT_EMIT_WARN(callbacks, cid, "rate-limiting", "Could not find metric-feedback channel.");
    return;
  }
  ...
  if (result <= 0) {
    ...
    if (ellapsed < 100) {
      O2_SIGNPOST_EVENT_EMIT_WARN(callbacks, cid, "rate-limiting",
                                  "Warning fairmq returned %llu earlier than expected. Sleeping %llu ms more before, retrying.",
                                  result, ellapsed);
      uv_sleep(100 - ellapsed);
    } else {
      O2_SIGNPOST_EVENT_EMIT(callbacks, cid, "rate-limiting",
                             "Error %llu, unable to send last consumed timeslice to source for %llu ms, retrying.", result, ellapsed);
    }
