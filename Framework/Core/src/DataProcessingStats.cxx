...
DataProcessingStats::DataProcessingStats(std::function<void(int64_t& base, int64_t& offset)> getRealtimeBase,
                                         std::function<int64_t(int64_t base, int64_t offset)> getTimestamp)
  : getTimestamp(std::move(getTimestamp)),
    getRealtimeBase(std::move(getRealtimeBase))
{
  getRealtimeBase(realTimeBase, initialTimeOffset);
}

std::function<void(int64_t&, int64_t&)> DataProcessingStatsHelpers::defaultRealtimeBaseConfigurator(uv_loop_t* loop)
{
  return [loop](int64_t& base, int64_t& offset) {
    uv_update_time(loop);
    base = uv_now(loop);
    offset = uv_hrtime();
  };
