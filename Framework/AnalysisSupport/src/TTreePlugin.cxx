    ...
    for (size_t mi = 0; mi < mappings.size(); ++mi) {
      ...
      if (listType) {
        auto vdata = std::make_shared<arrow::ArrayData>(datasetField->type()->field(0)->type(), valueOp.rootBranchEntries * valueOp.listSize,
                                                        std::vector<std::shared_ptr<arrow::Buffer>>{valueOp.targetBuffer});
        ...
        auto vdata = std::make_shared<arrow::ArrayData>(datasetField->type()->field(0)->type(), valueOp.offsetCount,
                                                        std::vector<std::shared_ptr<arrow::Buffer>>{valueOp.targetBuffer});
