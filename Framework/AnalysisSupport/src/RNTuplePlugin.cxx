  ...
  auto generator = [pool = options->pool, ntupleFragment, dataset_schema, &totalCompressedSize = mTotCompressedSize,
    ...
    for (auto& physicalField : fields) {
      ...
      if (physicalField->type() == arrow::boolean() ||
        ...
        switch (listSize) {
          case -1: {
            auto vdata = std::make_shared<arrow::ArrayData>(physicalField->type()->field(0)->type(), totalSize, std::vector<std::shared_ptr<arrow::Buffer>>{arrowValuesBuffer});
          ...
          case 1: {
            ...
            auto data = std::make_shared<arrow::ArrayData>(physicalField->type(), readEntries, std::vector<std::shared_ptr<arrow::Buffer>>{arrowValuesBuffer});
