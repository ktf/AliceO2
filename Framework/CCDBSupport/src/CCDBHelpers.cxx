                       ...
                       DataAllocator& allocator) -> void
{
  ...
  for (auto& route : helper->routes) {
    ...
    if (url2uuid != helper->mapURL2UUID.end()) {
      ...
      // when online.
      bool cacheExipred = (validUntil <= timestamp) && (timestamp <= cachePopulatedAt);
      checkValidity = (std::abs(int(timingInfo.tfCounter - url2uuid->second.lastCheckedTF)) >= chRate) && (isOnline || cacheExipred);
    } else {
