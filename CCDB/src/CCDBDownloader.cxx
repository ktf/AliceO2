...
void uvErrorCheck(int code)
{
  if (code != 0) {
    ...
    O2_SIGNPOST_EVENT_EMIT_ERROR(ccdb, sid, "CCDBDownloader", "UV error - %{public}s", buf);
  }
}
...
void curlEasyErrorCheck(CURLcode code)
{
  if (code != CURLE_OK) {
    O2_SIGNPOST_ID_GENERATE(sid, ccdb);
    O2_SIGNPOST_EVENT_EMIT_ERROR(ccdb, sid, "CCDBDownloader", "CURL error - %{public}s", curl_easy_strerror(code));
  }
}
...
void curlMultiErrorCheck(CURLMcode code)
{
  if (code != CURLM_OK) {
    O2_SIGNPOST_ID_GENERATE(sid, ccdb);
    O2_SIGNPOST_EVENT_EMIT_ERROR(ccdb, sid, "CCDBDownloader", "CURL error - %{public}s", curl_multi_strerror(code));
  }
}
namespace
{
...
void CCDBDownloader::closesocketCallback(void* clientp, curl_socket_t item)
{
  ...
  if (CD->mExternalLoop) {
    ...
    if (CD->mSocketTimerMap.find(item) != CD->mSocketTimerMap.end()) {
      ...
      if (close(item) == -1) {
        O2_SIGNPOST_ID_GENERATE(sid, ccdb);
        O2_SIGNPOST_EVENT_EMIT_ERROR(ccdb, sid, "CCDBDownloader", "CCDBDownloader: Socket failed to close");
      }
    }
  } else {
    if (close(item) == -1) {
...
curl_socket_t opensocketCallback(void* clientp, curlsocktype purpose, struct curl_sockaddr* address)
{
  ...
  if (sock == -1) {
    O2_SIGNPOST_ID_GENERATE(sid, ccdb);
    O2_SIGNPOST_EVENT_EMIT_ERROR(ccdb, sid, "CCDBDownloader", "CCDBDownloader: Socket failed to open");
  }
